...
セパレータ行の形式で配置方式を指定します。形式は処理後も保持されます：

- 左寄せ: `:----` または `----` （コロンなし）
  - 配置は同じで、セパレータの形式のみ異なります
...
セルの内容にパイプ文字 `|` を含める場合は、バックスラッシュでエスケープしてください：
...
エスケープされたパイプは、フォーマット処理で列の区切りとしては扱われません。
